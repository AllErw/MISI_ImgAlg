		...
		Note: setting N = 0 results in "nearest neighbour" interpolation.	*/
{
	...
	for (kxcnt = 0; kxcnt < Nsrc; kxcnt++) {
		for (kzcnt = 0; kzcnt < floor(Nt / 2.0); kzcnt++) {
	...
	for (kzcnt = 1; kzcnt < ceil(Nt / 2.0); kzcnt++) {
		// fill kx = 0 line:
		kxcnt = 0;
		P0[kxcnt + (Nt - kzcnt) * Nsrc][0] =        P0[kxcnt + kzcnt * Nsrc][0];
		P0[kxcnt + (Nt - kzcnt) * Nsrc][1] = -1.0 * P0[kxcnt + kzcnt * Nsrc][1];

		// fill kx != 0 lines:
		for (kxcnt = 1; kxcnt < Nsrc; kxcnt++) {
			P0[(Nsrc - kxcnt) + (Nt - kzcnt) * Nsrc][0] =        P0[kxcnt + kzcnt * Nsrc][0];
			P0[(Nsrc - kxcnt) + (Nt - kzcnt) * Nsrc][1] = -1.0 * P0[kxcnt + kzcnt * Nsrc][1];
		}
	}
